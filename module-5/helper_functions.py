...
def load_data(folder, n_files=100):
    """
    Loads up to n_files JSON files from the given folder, extracting title and body text.
    ...
    for file in file_list[:n_files]:
        try:
            with open(file, 'r') as f:
                paper = json.load(f)
                title = paper.get('title', '') or paper.get('metadata', {}).get('title', '')
...
def display_topics(model, feature_names, no_top_words):
    """
    Print the top words for each topic from an LDA model.
    """
